...
export default {
  ...
  async created() {
    const forgeCost = await this.contracts.RotInHell.methods.mintWeaponFee().call({ from: this.defaultAccount });
    const hellForgeCost = await this.contracts.RotInHell.methods.usdToHell(forgeCost).call();
    this.forgeCost = BN(hellForgeCost).div(BN(10).pow(18)).toFixed(4);

    const reforgeCost = await this.contracts.RotInHell.methods.reforgeWeaponFee().call({ from: this.defaultAccount });
    const hellReforgeCost = await this.contracts.RotInHell.methods.usdToHell(reforgeCost).call();
