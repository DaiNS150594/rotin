...
export default {
  ...
  methods: {
    ...mapActions(['fetchTargets', 'doEncounter', 'fetchFightRewardHell', 'fetchFightRewardXp', 'getXPRewardsIfWin']),
    ...
    getWinChance(enemyPower, enemyElement) {
      ...
      if (playerMin > enemyMax) return 'High Chance of';
      if (playerMax < enemyMin) return 'Low Chance of';
      ...
      if (rollingTotal <= 0.3) return 'Low Chance of';
      if (rollingTotal <= 0.5) return 'Possible';
      if (rollingTotal <= 0.7) return 'Favorable';
    ...
    async onClickEncounter(targetToFight) {
      ...
      try {
        ...
        await this.fetchFightRewardHell();
    ...
    formattedHell(hell) {
      const hellBalance = Web3.utils.fromWei(hell, 'ether');
    ...
    getPotentialXp(targetToFight) {
      ...
      //Formula taken from getXpGainForFight funtion of ROTINHELL.sol
