    ...
    actions: {
      ...
      setUpContractEvents({ state, dispatch, commit }) {
        ...
        if (!featureFlagStakeOnly) {
          ...
          subscriptions.push(
            state.contracts().RotInHell!.events.FightOutcome({ filter: { owner: state.defaultAccount } }, async (err: Error, data: any) => {
              if (err) {
                console.error(err, data);
                return;
              }
          ...
          subscriptions.push(
            state.contracts().RotInHell!.events.InGameOnlyFundsGiven({ filter: { to: state.defaultAccount } }, async (err: Error, data: any) => {
              if (err) {
                console.error(err, data);
                return;
              }
      ...
      async fetchUserGameDetails({ state, dispatch, commit }) {
        ...
        const [
          ...
          state.contracts().RotInHell!.methods.getMyCharacters().call(defaultCallOptions(state)),
          state.contracts().RotInHell!.methods.getMyWeapons().call(defaultCallOptions(state)),
      ...
      async updateWeaponIds({ state, dispatch, commit }) {
        ...
        const ownedWeaponIds = await state.contracts().RotInHell!.methods.getMyWeapons().call(defaultCallOptions(state));
        commit('updateUserDetails', {
          ownedWeaponIds: Array.from(ownedWeaponIds)
        });
      ...
      async updateCharacterIds({ state, dispatch, commit }) {
        ...
        const ownedCharacterIds = await state.contracts().RotInHell!.methods.getMyCharacters().call(defaultCallOptions(state));
        commit('updateUserDetails', {
          ownedCharacterIds: Array.from(ownedCharacterIds)
        });
      ...
      async fetchInGameOnlyFunds({ state, commit }) {
        const { RotInHell } = state.contracts();
        if(!RotInHell || !state.defaultAccount) return;

        const inGameOnlyFunds = await RotInHell.methods
      ...
      async addMoreHell({ state, dispatch }, hellToAdd: string) {
        ...
        await state.contracts().RotInHell!.methods.recoverHell(hellToAdd).send({
      ...
      async mintCharacter({ state, dispatch }) {
        ...
        await state.contracts().RotInHell!.methods.mintCharacter().send(defaultCallOptions(state));
      ...
      async mintWeapon({ state, dispatch }) {
        ...
        await state.contracts().RotInHell!.methods.mintWeapon().send({
          from: state.defaultAccount,
        });
      ...
      async reforgeWeapon({ state, dispatch }, { burnWeaponId, reforgeWeaponId }) {
        ...
        await state.contracts().RotInHell!.methods
          .reforgeWeapon(
            reforgeWeaponId,
            burnWeaponId
          )
      ...
      async fetchTargets({ state, commit }, { characterId, weaponId }) {
        ...
        const targets = await state.contracts().RotInHell!.methods
      ...
      async doEncounter({ state, dispatch }, { characterId, weaponId, targetString }) {
        ...
        const res = await state.contracts().RotInHell!.methods
          .fight(
            characterId,
            weaponId,
            targetString
      ...
      async fetchFightRewardHell({ state, commit, dispatch }) {
        ...
        const [hellRewards] = await Promise.all([
          (async () => {
            const hellRewards = await RotInHell.methods
              .getTokenRewards()
              .call(defaultCallOptions(state));
      ...
      async fetchFightRewardXp({ state, commit }) {
        ...
        const xpCharaIdPairs = await Promise.all(
          state.ownedCharacterIds.map(async charaId => {
            const xp = await RotInHell.methods
      ...
      async claimTokenRewards({ state, dispatch }) {
        ...
        await RotInHell.methods.claimTokenRewards().send({
          from: state.defaultAccount,
        });
      ...
      async claimXpRewards({ state, dispatch }) {
        ...
        await RotInHell.methods.claimXpRewards().send({
          from: state.defaultAccount,
        });
