...
import { abi as rotinHellAbi, networks as rotinHellNetworks } from '../../build/contracts/RotInHell.json';
...
export async function setUpContracts(web3: Web3): Promise<Contracts> {
  ...
  const rotinHellContractAddr = process.env.VUE_APP_ROTINHELL_CONTRACT_ADDRESS || (rotinHellNetworks as Networks)[networkId]!.address;

  const RotInHell = new web3.eth.Contract(rotinHellAbi as Abi, rotinHellContractAddr);
  const [charactersAddr, weaponsAddr, randomsAddr] = await Promise.all([
    RotInHell.methods.characters().call(),
    RotInHell.methods.weapons().call(),
    RotInHell.methods.randoms().call(),
  ]);
  ...
  return {
    ...stakingContracts,
    RotInHell, Randoms, Characters, Weapons,
    ...raidContracts,
    ...marketContracts,
    WaxBridge,
  };
